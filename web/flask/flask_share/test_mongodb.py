...
def list_all_collections(db_name):
    global mongo_client

def list_all_keys(db_name, collection_name):
...
def list_all_keys_types(db_name, collection_name):
    db = mongo_client[db_name]
    map = Code("function() { for (var key in this) { emit(key, this[key]); } }")
    reduce = Code("function(key, stuff) { return typeof(stuff).toString(); }")
    result = db[collection_name].map_reduce(map, reduce, "myresults")
    result2 = pd.DataFrame(list(db[collection_name].map_reduce(map, reduce, "myresults").find({})))
    print(result2)
    return result.distinct('_id')

def print_content(db_name, collection_name):
    mongo_db     = mongo_client[db_name]
    mongo_cursor = mongo_db[collection_name].find({}, {'_id':0}) # filter out the field '_id'
...
def print_certain_content(db_name, collection_name, field_names):
    mongo_db     = mongo_client[db_name]
    field_filter = {'_id':0}
    for key in field_names:
        field_filter[key] = 1
    mongo_cursor = mongo_db[collection_name].find({}, field_filter) # filter out the field '_id'
    df_mongo     = pd.DataFrame(list(mongo_cursor))
    print(df_mongo)
    '''
    df_mongo     = df_mongo.drop(columns=['_id'])
